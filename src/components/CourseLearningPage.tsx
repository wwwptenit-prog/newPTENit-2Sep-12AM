...
export const CourseLearningPage: React.FC<CourseLearningPageProps> = ({
  ...
  return (
    <div className="bg-slate-950 text-white min-h-screen flex flex-col font-sans selection:bg-[#006A4E] selection:text-white">
      ...
      <header className="bg-[#006A4E] text-white border-b border-[#00543D] px-3.5 sm:px-6 py-2.5 flex items-center justify-between gap-3 sticky top-0 z-40 shadow-md">
        <div className="flex items-center gap-3 min-w-0">
          <button
            type="button"
            onClick={onBack}
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-[#00543D] hover:bg-[#00412F] text-white transition font-bold text-xs cursor-pointer shrink-0 border border-emerald-600/40"
          ...
          <div className="min-w-0">
            ...
            <p className="text-[11px] text-emerald-100 font-medium truncate">
              ইন্সট্রাকটর: <span className="text-white font-bold">{typeof course.instructor === 'object' ? (course.instructor as any)?.name : (course.instructor || 'প্রকৌশলী আল-আমিন')}</span>
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 shrink-0">
          <button
            ...
            className="lg:hidden px-3 py-1.5 rounded-xl bg-[#00543D] hover:bg-[#00412F] text-white font-bold text-xs flex items-center gap-1.5 border border-emerald-600/50 cursor-pointer"
          >
