...
export const NotificationCenterModal: React.FC<NotificationCenterModalProps> = ({
  ...
  useEffect(() => {
    if (isNotificationCenterOpen || isEmbedded) {
  ...
  }, [isNotificationCenterOpen, isEmbedded, currentUser]);

  if (!isNotificationCenterOpen && !isEmbedded) return null;
  if (!currentUser) return null;
  if (isMessengerInboxOpen) return null;
  ...
  const content = (
    <div className={`w-full bg-white dark:bg-slate-900 text-slate-900 dark:text-white flex flex-col overflow-hidden font-bengali ${
      isEmbedded
        ? "border border-slate-200/90 dark:border-slate-800 rounded-2xl sm:rounded-3xl shadow-xs min-h-[580px] sm:min-h-[660px] animate-fadeIn"
        : "h-[100dvh] sm:h-[650px] sm:max-w-lg sm:rounded-3xl shadow-2xl border-0 sm:border sm:border-slate-200"
    }`}>
      {/* HEADER BAR */}
      <div className="p-3.5 sm:p-4 bg-slate-50/90 dark:bg-slate-800/60 border-b border-slate-200 dark:border-slate-800 flex flex-col gap-2 shrink-0">
        <div className="flex items-center justify-between">
          {selectedNotification ? (
            <button
              type="button"
              onClick={() => setSelectedNotification(null)}
              className="flex items-center gap-1.5 text-slate-700 dark:text-slate-200 hover:text-slate-900 dark:hover:text-white transition cursor-pointer active:scale-95 py-1 px-2.5 rounded-xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 hover:bg-slate-100 dark:hover:bg-slate-700"
            >
              <ChevronLeft className="w-5 h-5 text-[#006A4E] dark:text-emerald-400 stroke-[2.5]" />
            ...
            <div className="flex items-center gap-2.5">
              <button
                type="button"
                onClick={handleClose}
                className="p-1.5 rounded-xl text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-white hover:bg-slate-200 dark:hover:bg-slate-750 transition cursor-pointer active:scale-95"
                title="ফিডে ফিরে যান"
              >
                <ChevronLeft className="w-6 h-6 stroke-[2.5] text-slate-600 dark:text-slate-300" />
              </button>

              <div className="flex items-center gap-2">
                <div className="p-2 rounded-xl bg-emerald-100 dark:bg-emerald-950/60 border border-emerald-200 dark:border-emerald-850 text-[#006A4E] dark:text-emerald-400">
                  <Bell className="w-5 h-5" />
                </div>
                <div>
                  <div className="flex items-center gap-1.5">
                    <h2 className="text-sm sm:text-base font-black text-slate-900 dark:text-white tracking-tight leading-none flex items-center gap-1.5">
          ...
          <div className="flex items-center gap-1.5 shrink-0">
            <button
              ...
              className="p-2 rounded-xl bg-white dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white transition cursor-pointer active:scale-95 border border-slate-200 dark:border-slate-700"
            ...
            <button
              type="button"
              onClick={handleClose}
              className="p-2 rounded-xl bg-rose-50 dark:bg-rose-950/40 hover:bg-rose-100 dark:hover:bg-rose-900/60 text-[#E11D48] transition cursor-pointer active:scale-95 border border-rose-200 dark:border-rose-850"
              title="বন্ধ করে ফিডে ফিরে যান"
            >
          ...
          <div className="w-full max-w-md mx-auto">
            <div className="relative flex items-center">
              ...
              <input
                ...
                className="w-full pl-10 pr-9 py-2.5 bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-white placeholder-slate-400 font-medium text-xs sm:text-sm rounded-xl shadow-xs border border-slate-200 dark:border-slate-700 focus:outline-none focus:ring-2 focus:ring-[#006A4E] focus:border-transparent transition-all"
              />
              {searchQuery && (
                <button
                  type="button"
                  onClick={() => setSearchQuery("")}
                  className="absolute right-3 top-1/2 -translate-y-1/2 w-5 h-5 rounded-full bg-slate-200 dark:bg-slate-700 hover:bg-slate-300 dark:hover:bg-slate-600 text-slate-600 dark:text-slate-300 flex items-center justify-center text-xs transition cursor-pointer"
                  title="মুছে ফেলুন"
                >
                  <X className="w-3 h-3" />
                </button>
        ...
        <div className="flex-1 overflow-y-auto divide-y divide-slate-100 dark:divide-slate-800/80 w-full bg-white dark:bg-slate-900 min-h-[380px]">
          {filteredNotifications.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-64 text-center p-6 space-y-3">
              <div className="w-16 h-16 rounded-3xl bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 flex items-center justify-center text-slate-400 dark:text-slate-500">
            ...
            <div className="p-4 sm:p-5 flex flex-col h-full bg-white dark:bg-slate-900 animate-in fade-in duration-200 overflow-y-auto">
              <div className="space-y-4 max-w-lg mx-auto w-full">
                <div className="flex items-start gap-3.5 p-3.5 rounded-2xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-750">
                  {getNotifIcon(selectedNotification)}
                  <div className="flex-1 min-w-0">
                    <h3 className="text-sm font-black text-slate-900 dark:text-white leading-tight">
                      {selectedNotification.title}
                    </h3>
                    <p className="text-[11px] font-bold text-slate-500 dark:text-slate-400 mt-1 font-sans">
                      {selectedNotification.time}
                    </p>
                  </div>
                </div>

                <div className="p-4 rounded-2xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-750 text-xs sm:text-sm text-slate-700 dark:text-slate-200 leading-relaxed font-sans whitespace-pre-wrap">
                ...
                <div className="pt-2 flex items-center gap-3">
                  ...
                  <button
                    ...
                    className="p-3 rounded-xl bg-rose-50 dark:bg-rose-950/40 hover:bg-rose-100 dark:hover:bg-rose-900/60 text-[#E11D48] font-bold text-xs transition cursor-pointer border border-rose-200 dark:border-rose-800"
                    title="মুছে ফেলুন"
                  >
              ...
              <div
                ...
                className={`p-3.5 sm:px-4 sm:py-3.5 flex items-start gap-3 cursor-pointer transition-colors w-full ${
                  !notif.read
                    ...
                    : "hover:bg-slate-50 dark:hover:bg-slate-800/50 opacity-90"
                }`}
              >
                {getNotifIcon(notif)}
                <div className="flex-1 min-w-0 space-y-1">
                  <div className="flex items-center justify-between gap-1">
                    <h4
                      className={`text-xs sm:text-sm font-black truncate ${
                        !notif.read ? "text-[#006A4E] dark:text-emerald-400" : "text-slate-800 dark:text-slate-200"
                      }`}
                    >
                      {notif.title}
                    </h4>
                    <span className="text-[10px] text-slate-500 dark:text-slate-400 shrink-0 font-bold ml-1 font-sans">
                      {notif.time}
                    </span>
                  </div>
                  <p className="text-xs text-slate-600 dark:text-slate-300 leading-relaxed line-clamp-2 font-sans">
                    {notif.message}
                  </p>
                  ...
                  <div className="flex items-center justify-between pt-2 border-t border-slate-100 dark:border-slate-800/80 mt-1">
                    <div className="flex items-center gap-2">
                      <span
                        className={`text-[10px] font-bold px-2 py-0.5 rounded-md ${
                          !notif.read
                            ? "bg-emerald-100 dark:bg-emerald-950/60 text-[#006A4E] dark:text-emerald-400"
                      ...
                      <button
                        ...
                        className="text-[11px] font-black text-[#006A4E] dark:text-emerald-400 hover:underline flex items-center gap-0.5 cursor-pointer"
                      >
                        <span>ওপেন</span>
                    ...
                    <button
                      ...
                      className="p-1.5 text-slate-400 dark:text-slate-500 hover:text-[#E11D48] hover:bg-rose-50 dark:hover:bg-slate-800 rounded-lg transition cursor-pointer"
                      title="মুছে ফেলুন"
                    >
                      <Trash2 className="w-3.5 h-3.5" />
                    </button>
      ...
      {isNotifSettingsOpen && (
        ...
        >
          <div 
            ...
            className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 w-full max-w-sm rounded-2xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-150">
            <div className="px-5 py-4 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between">
              <h3 className="font-black text-base text-slate-900 dark:text-white flex items-center gap-2">
                <Settings className="w-5 h-5 text-[#006A4E] dark:text-emerald-400" />
                <span>নোটিফিকেশন পছন্দসমূহ</span>
              </h3>
              <button
                type="button"
                onClick={() => setIsNotifSettingsOpen(false)}
                className="p-1.5 rounded-full bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white transition cursor-pointer"
                title="বন্ধ করুন"
              >
                <X className="w-5 h-5" />
              </button>
            ...
            <div className="p-5 space-y-3.5 text-xs font-semibold text-slate-700 dark:text-slate-300">
              <div className="flex items-center justify-between p-3 rounded-xl bg-slate-50 dark:bg-slate-800/70 border border-slate-200 dark:border-slate-700">
                <div>
                  ...
                  <div className="text-[11px] text-slate-500 dark:text-slate-400">নতুন ডেলিভারি ও কাজের অগ্রগতি অ্যালার্ট</div>
                </div>
                <input
                  type="checkbox"
              ...
              <div className="flex items-center justify-between p-3 rounded-xl bg-slate-50 dark:bg-slate-800/70 border border-slate-200 dark:border-slate-700">
                <div>
                  ...
                  <div className="text-[11px] text-slate-500 dark:text-slate-400">সেলার ও বায়ার থেকে নতুন বার্তার নোটিশ</div>
                </div>
                <input
                  type="checkbox"
              ...
              <div className="flex items-center justify-between p-3 rounded-xl bg-slate-50 dark:bg-slate-800/70 border border-slate-200 dark:border-slate-700">
                <div>
                  ...
                  <div className="text-[11px] text-slate-500 dark:text-slate-400">নতুন লেকচার, লাইভ ক্লাস ও অ্যাসাইনমেন্ট অ্যালার্ট</div>
                </div>
                <input
                  type="checkbox"
              ...
              <div className="flex items-center justify-between p-3 rounded-xl bg-slate-50 dark:bg-slate-800/70 border border-slate-200 dark:border-slate-700">
                <div>
                  ...
                  <div className="text-[11px] text-slate-500 dark:text-slate-400">নোটিফিকেশন এলে নোটিফিকেশন সাউন্ড বাজবে</div>
                </div>
                <input
                  type="checkbox"
